...
def run_ohlcv_ingest(
    ...
    for i, sym in enumerate(symbols, 1):
        try:
            ...
            rows = writer.write_ohlcv(df, symbol=sym, batch_id=batch_id, source=source)
    ...
    print(f"\n==> Batch {batch_id}: {ok}/{len(symbols)} ma OK. Manifest: {manifest_path}")
    return {"batch_id": batch_id, "results": results, "manifest": manifest_path}
